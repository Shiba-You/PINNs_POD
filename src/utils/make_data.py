...
def make_data(**kwargs):
    ...
    kwargs = {
        ...
        data_form : データの保存様式  True : U,C,p,vorticity   False : u,v,x,y,p,t
    }
    ...
    '''
    U_star, P_star                       = read_data(kwargs["path"], kwargs["data_form"])
...
def read_data(path, data_form):
    ...
    elif data_form == False:
        u_star = pd.read_csv(path + 'u_arrange_data.csv', header = None, sep = '\s+').values
        v_star = pd.read_csv(path + 'v_arrange_data.csv', header = None, sep = '\s+').values
        P_star = pd.read_csv(path + 'p_arrange_data.csv', header = None, sep = '\s+').values
        U_star = np.c_[u_star, v_star]
